...
function parse(buffer: AnyBuffer): Header {
  ...
  const fileSignature = AnyBuffer.toString(AnyBuffer.slice(dv, 0, 4))
  ...
  return {
    ...
    projectId: formatGuid(AnyBuffer.slice(dv, 8, 24)),
    majorVersion,
    minorVersion,
    // systemIdentifier: parseCString(new DataView(dv.buffer, 26, 58)),
    // generatingSoftware: parseCString(new DataView(dv.buffer, 58, 90)),
    systemIdentifier: AnyBuffer.toString(AnyBuffer.slice(dv, 26, 58)),
    generatingSoftware: AnyBuffer.toString(AnyBuffer.slice(dv, 58, 90)),
    ...
    pointCountByReturn: parseNumberOfPointsByReturn(
      AnyBuffer.slice(dv, 255, 375)
    ),
    scale: parsePoint(AnyBuffer.slice(dv, 131, 155)),
    offset: parsePoint(AnyBuffer.slice(dv, 155, 179)),
    min: [
...
function parseNumberOfPointsByReturn(dv: DataView): number[] {
  const bigs: BigInt[] = []
